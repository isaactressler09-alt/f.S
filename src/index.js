...
function loadDb() {
  ...
  if (!fs.existsSync(DB_FILE)) {
    const initial = {
      settings: {
        ...
        calendarDashboardMessageId: null,
  ...
  if (!('calendarDashboardMessageId' in db.settings)) db.settings.calendarDashboardMessageId = null;
...
function closedDayFor(db, date) {
  return (db.closedDays || []).find(day => day.date === date) || null;
}

function isDateClosed(db, date) {
  return Boolean(closedDayFor(db, date));
}
...
function calendarDaySummary(db, date) {
  const jobs = (db.schedules || [])
    .filter(job => job.date === date)
    .filter(job => job.status !== 'cancelled' && job.status !== 'completed');
  const clientJobs = jobs.filter(job => job.kind === 'client').length;
  const workerShifts = jobs.filter(job => job.kind === 'worker').length;
  const closed = closedDayFor(db, date);
  return { clientJobs, workerShifts, closed };
}

function calendarCellText(day, summary) {
  if (summary.closed) return `${String(day).padStart(2, '0')} 🚫`;
  const pieces = [];
  if (summary.clientJobs) pieces.push(`${summary.clientJobs}J`);
  if (summary.workerShifts) pieces.push(`${summary.workerShifts}W`);
  return `${String(day).padStart(2, '0')}${pieces.length ? ` ${pieces.join('/')}` : ''}`;
}

function calendarGridText(year, month) {
  const db = loadDb();
  ...
  for (let week = 0; week < 6; week++) {
    ...
    for (let dow = 0; dow < 7; dow++) {
      if ((week === 0 && dow < startDay) || day > daysInMonth) {
        ...
        const cell = calendarCellText(day, calendarDaySummary(db, date));
        cells.push(cell.padEnd(9, ' ').slice(0, 9));
        day++;
      }
    }
...
function calendarMonthItems(year, month, limit = 18) {
  const db = loadDb();
  const prefix = monthDatePrefix(year, month);
  const jobs = (db.schedules || [])
    .filter(job => job.date?.startsWith(prefix))
    .filter(job => job.status !== 'cancelled' && job.status !== 'completed')
    .sort((a, b) => `${a.date} ${a.time}`.localeCompare(`${b.date} ${b.time}`))
    .slice(0, limit)
    .map(job => `• **${job.date} ${job.time}** — ${job.kind === 'worker' ? 'Worker' : 'Client'} — <@${job.userId}> — ${serviceLabel(job.serviceKey)} \`${job.id}\``);

  const closed = (db.closedDays || [])
    .filter(day => day.date?.startsWith(prefix))
    .sort((a, b) => a.date.localeCompare(b.date))
    .map(day => `• **${day.date}** — 🚫 Closed: ${day.reason || 'No reason provided'}`);

  const items = [...closed, ...jobs].sort((a, b) => a.localeCompare(b));
  return items.length ? items.join('\n').slice(0, 3900) : 'No jobs, worker shifts, or closed days listed for this month.';
}

function calendarMonthEmbed(year, month) {
  return new EmbedBuilder()
    .setTitle(`📆 FreshStart Calendar • ${monthName(month)} ${year}`)
    .setColor(0x5865f2)
    .setDescription([
      '```',
      calendarGridText(year, month),
      '```',
      '**Legend:** J = client jobs • W = worker shifts • 🚫 = closed/holiday',
      '',
      calendarMonthItems(year, month)
    ].join('\n'))
    .setFooter({ text: 'Use /calendar-day date:YYYY-MM-DD for full details. Use /calendar-close to block holidays/closed days.' })
    .setTimestamp();
}

function makeCalendarDashboardEmbeds(view = 'two_months') {
  const { year, month } = currentEasternMonthYear();
  const embeds = [calendarMonthEmbed(year, month)];
  if (view === 'two_months') {
    const next = addMonths(year, month, 1);
    embeds.push(calendarMonthEmbed(next.year, next.month));
  }
  return embeds;
}

async function refreshCalendar(guild, view = 'two_months') {
  const db = loadDb();
  const channel = await fetchChannel(guild, CONFIG.channels.scheduling);
  if (!channel) return;
  const message = await postOrEditMessage(channel, db.settings.calendarDashboardMessageId, {
    embeds: makeCalendarDashboardEmbeds(view)
  });
  db.settings.calendarDashboardMessageId = message.id;
  saveDb(db);
}

function calendarDayDetailsEmbed(date) {
  const db = loadDb();
  const closed = closedDayFor(db, date);
  const jobs = (db.schedules || [])
    .filter(job => job.date === date)
  ...
  const activeJobs = jobs.filter(job => job.status !== 'cancelled' && job.status !== 'completed');
  const cancelledJobs = jobs.filter(job => job.status === 'cancelled');
  const completedJobs = jobs.filter(job => job.status === 'completed');

  const activeText = activeJobs.length ? activeJobs.map(jobLine).join('\n').slice(0, 1800) : 'No active jobs or worker shifts for this day.';
  const oldText = [...cancelledJobs, ...completedJobs].length
    ? [...completedJobs, ...cancelledJobs].slice(0, 10).map(job => `• ${job.status.toUpperCase()} — <@${job.userId}> — ${serviceLabel(job.serviceKey)} \`${job.id}\``).join('\n')
    : 'None';

  const embed = new EmbedBuilder()
    .setTitle(`📆 FreshStart Calendar Day • ${date}`)
    .setColor(closed ? 0xe74c3c : 0x5865f2)
    .addFields(
      { name: 'Closed / Holiday?', value: closed ? `🚫 Yes — ${closed.reason || 'No reason provided'}` : 'No', inline: false },
      { name: 'Active Schedule', value: activeText, inline: false },
      { name: 'Completed/Cancelled', value: oldText.slice(0, 1024), inline: false }
    )
    .setTimestamp();
  return embed;
}

async function deleteLegacyJobBoardMessage(guild, job) {
  if (!job?.jobBoardMessageId) return;
  ...
  job.jobBoardMessageId = null;
}

function markInteractionProcessed(interaction) {
  const db = loadDb();
  db.processedInteractions ||= [];
  if (db.processedInteractions.includes(interaction.id)) return false;
  db.processedInteractions.push(interaction.id);
  db.processedInteractions = db.processedInteractions.slice(-300);
  saveDb(db);
  return true;
}
...
async function handleScheduleAdd(interaction) {
  ...
  const db = loadDb();
  if (isDateClosed(db, date)) {
    const closed = closedDayFor(db, date);
    await interaction.reply({
      content: `🚫 That date is marked closed/holiday: **${closed.reason || 'No reason provided'}**. Use \`/calendar-open\` first if you want to schedule anyway.`,
      ephemeral: true
    });
    return;
  }
  ...
  await refreshCalendar(interaction.guild).catch(() => null);
...
async function handleScheduleUpdate(interaction) {
  ...
  if (newDate && isDateClosed(db, newDate)) {
    const closed = closedDayFor(db, newDate);
    await interaction.reply({ content: `🚫 That date is marked closed/holiday: **${closed.reason || 'No reason provided'}**. Use \`/calendar-open\` first if you want to schedule there.`, ephemeral: true });
    return;
  }
  ...
  await deleteLegacyJobBoardMessage(interaction.guild, job).catch(() => null);
...
async function handleScheduleRemovePerson(interaction) {
  const member = await interaction.guild.members.fetch(interaction.user.id);
  if (!isManagerPlus(member)) {
    await interaction.reply({ content: 'Only Manager/Admin/Owner can remove schedules.', ephemeral: true });
    return;
  ...
  const db = loadDb();
  const matches = db.schedules.filter(job => {
    if (job.userId !== person.id) return false;
  ...
  await refreshCalendar(interaction.guild).catch(() => null);
  await updateMoneyDashboard(interaction.guild).catch(() => null);
  await logEvent(interaction.guild, 'Person Removed From Schedule', `${interaction.user} removed/cancelled ${matches.length} schedule entries for ${person}.`, [
    { name: 'Date Filter', value: date || 'All upcoming dates', inline: true },
    { name: 'Kind Filter', value: kind || 'Client + Worker', inline: true },
    { name: 'Reason', value: reason, inline: false }
  ]);

  await interaction.reply({ content: `❌ Removed/cancelled **${matches.length}** schedule entr${matches.length === 1 ? 'y' : 'ies'} for ${person}.`, ephemeral: true });
}

async function handleCalendarPost(interaction) {
  const member = await interaction.guild.members.fetch(interaction.user.id);
  if (!isManagerPlus(member)) {
    await interaction.reply({ content: 'Only Manager/Admin/Owner can post the calendar dashboard.', ephemeral: true });
    return;
  }
  const view = interaction.options.getString('view') || 'two_months';
  await refreshCalendar(interaction.guild, view).catch(error => { throw error; });
  await logEvent(interaction.guild, 'Calendar Dashboard Posted', `${interaction.user} posted/refreshed the calendar dashboard.`, [
    { name: 'View', value: view, inline: true }
  ]);
  await interaction.reply({ content: `✅ Calendar dashboard updated in <#${CONFIG.channels.scheduling}>.`, ephemeral: true });
}

async function handleCalendarView(interaction) {
  const member = await interaction.guild.members.fetch(interaction.user.id);
  if (!isHelperPlus(member)) {
    await interaction.reply({ content: 'Only Helper or higher can view the calendar.', ephemeral: true });
    return;
  }
  ...
  await interaction.reply({ embeds: [calendarMonthEmbed(year, month)], ephemeral: true });
}

async function handleCalendarDay(interaction) {
  const member = await interaction.guild.members.fetch(interaction.user.id);
  if (!isHelperPlus(member)) {
    await interaction.reply({ content: 'Only Helper or higher can view calendar day details.', ephemeral: true });
    return;
  }
  const date = interaction.options.getString('date', true);
  if (!parseDate(date)) {
  ...
  await interaction.reply({ embeds: [calendarDayDetailsEmbed(date)], ephemeral: true });
}

async function handleCalendarClose(interaction) {
  const member = await interaction.guild.members.fetch(interaction.user.id);
  if (!isManagerPlus(member)) {
    await interaction.reply({ content: 'Only Manager/Admin/Owner can mark closed days.', ephemeral: true });
    return;
  }
  const date = interaction.options.getString('date', true);
  ...
  const db = loadDb();
  db.closedDays = (db.closedDays || []).filter(day => day.date !== date);
  db.closedDays.push({
    date,
    reason,
    createdBy: interaction.user.id,
  ...
  await refreshCalendar(interaction.guild).catch(() => null);
  await logEvent(interaction.guild, 'Calendar Closed Day Added', `${interaction.user} marked ${date} as closed.`, [
    { name: 'Reason', value: reason }
  ]);
  await interaction.reply({ content: `🚫 Marked **${date}** as closed/holiday: **${reason}**`, ephemeral: true });
}

async function handleCalendarOpen(interaction) {
  const member = await interaction.guild.members.fetch(interaction.user.id);
  if (!isManagerPlus(member)) {
    await interaction.reply({ content: 'Only Manager/Admin/Owner can reopen days.', ephemeral: true });
    return;
  }
  const date = interaction.options.getString('date', true);
  if (!parseDate(date)) {
  ...
  const db = loadDb();
  const before = (db.closedDays || []).length;
  db.closedDays = (db.closedDays || []).filter(day => day.date !== date);
  const removed = before - db.closedDays.length;
  saveDb(db);

  await refreshCalendar(interaction.guild).catch(() => null);
  await logEvent(interaction.guild, 'Calendar Day Reopened', `${interaction.user} reopened ${date}.`, [
    { name: 'Removed Closed Blocks', value: String(removed), inline: true }
  ]);
  await interaction.reply({ content: removed ? `✅ Reopened **${date}**.` : `No closed/holiday block was found for **${date}**.`, ephemeral: true });
}
...
client.once('ready', async () => {
  ...
  if (guild) {
    ...
    await refreshCalendar(guild).catch(error => console.error('Calendar refresh failed:', error));
...
client.on('interactionCreate', async interaction => {
  try {
    if (!markInteractionProcessed(interaction)) return;
    ...
    } else if (command === 'schedule-remove-person') {
      await handleScheduleRemovePerson(interaction);
    } else if (command === 'calendar-post') {
      await handleCalendarPost(interaction);
    } else if (command === 'calendar-view') {
      await handleCalendarView(interaction);
    } else if (command === 'calendar-day') {
      await handleCalendarDay(interaction);
    } else if (command === 'calendar-close') {
      await handleCalendarClose(interaction);
    } else if (command === 'calendar-open') {
      await handleCalendarOpen(interaction);
