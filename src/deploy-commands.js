...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName('schedule-remove-person')
    .setDescription('Cancel/remove upcoming schedule entries for a person.')
    .addUserOption(option => option
      .setName('person')
      .setDescription('Person to remove from schedule')
      .setRequired(true))
    .addStringOption(option => option
      .setName('date')
      .setDescription('Optional: only remove this date, YYYY-MM-DD')
      .setRequired(false))
    .addStringOption(option => option
      .setName('kind')
      .setDescription('Optional: client job or worker shift')
      .setRequired(false)
      .addChoices(
        { name: 'Client Job', value: 'client' },
        { name: 'Worker Shift / On Call', value: 'worker' }
  ...
  new SlashCommandBuilder()
    ...
    .setDescription('Post or refresh the scheduling calendar dashboard.')
    .addStringOption(option => option
      .setName('view')
      .setDescription('Calendar dashboard view')
      .setRequired(false)
      .addChoices(
        { name: 'Current Month', value: 'current' },
        { name: 'Current + Next Month', value: 'two_months' }
      )),

  new SlashCommandBuilder()
    .setName('calendar-view')
    .setDescription('View a FreshStart calendar month privately.')
    .addIntegerOption(option => option
      .setName('month')
      .setDescription('Month number, 1-12. Leave blank for current month.')
      .setRequired(false)
      .setMinValue(1)
      .setMaxValue(12))
    .addIntegerOption(option => option
      .setName('year')
      .setDescription('Year, for example 2026. Leave blank for current year.')
      .setRequired(false)
      .setMinValue(2024)
      .setMaxValue(2100)),

  new SlashCommandBuilder()
    .setName('calendar-day')
    .setDescription('View all jobs, worker shifts, and closure notes for one date.')
    .addStringOption(option => option
      .setName('date')
      .setDescription('Date in YYYY-MM-DD format')
      .setRequired(true)),
  ...
  new SlashCommandBuilder()
    .setName('calendar-close')
    .setDescription('Mark a date as closed/holiday/no booking.')
    .addStringOption(option => option
      .setName('date')
      .setDescription('Date in YYYY-MM-DD format')
      .setRequired(true))
    .addStringOption(option => option
      .setName('reason')
      .setDescription('Holiday or reason for closing')
      .setRequired(true)),

  new SlashCommandBuilder()
    .setName('calendar-open')
    .setDescription('Remove a closed/holiday block from a date.')
    .addStringOption(option => option
      .setName('date')
      .setDescription('Date in YYYY-MM-DD format')
      .setRequired(true)),
